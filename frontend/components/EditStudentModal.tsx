...
export default function EditStudentModal({ student, onClose, onSuccess }: { student: any; onClose: () => void; onSuccess: () => void }) {
  const [formData, setFormData] = useState({
    ...
    membership_end_date: String(student.membership_end_date).slice(0, 10),
    monthly_due_date: student.monthly_due_date.toString(),
  });
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                {(() => {
                  const b = batches.find((x) => x.name === (formData as any).batch);
                  return b ? `Available seats: ${b.available} / ${b.total_seats}` : 'Checking availability...';
                })()}
              </p>
            </div>
            <div>
